...
def _stage2_extract_bom(claude_client, pdf_b64, structure, bom_images=None):
    ...
    prompt += (
        ...
        "  ROW ALIGNMENT CHECK: For EVERY row, verify that part_number, description,\n"
        "  and qty all come from the SAME horizontal line in the table. If the part\n"
        "  number seems to belong to the row above or below, STOP — you have a row\n"
        ...
        "RULE 6 — SUB-ITEMS (extract in ONE pass, do NOT go back):\n"
        ...
        "  JSON object with item_num:0.\n"
        "  IMPORTANT — Sub-item rows have the EXACT same column layout as numbered rows:\n"
        "    Column 1 (Item No.) = BLANK\n"
        "    Column 2 (Part Number) = the sub-item's part number\n"
        "    Column 3 (Description) = the sub-item's description\n"
        "    Column 4 (Qty) = the sub-item's quantity\n"
        "  Do NOT let the blank item number shift your column reading.\n"
        "  The part number is ALWAYS in the part number column, even when item no. is blank.\n"
        "  Read the table TOP-TO-BOTTOM in a SINGLE PASS. When you hit a row with no\n"
        "  item number, output it immediately as item_num:0 with its own pn/desc/qty,\n"
        "  then continue to the next row. Do NOT go back and re-scan.\n"
        "  Common sub-item examples:\n"
        "    - End sections / end covers (e.g. 3030420 END SECTION D-ST4)\n"
        ...
        "For sub-items, use item_num:0 with the sub-item's own part_number, description, and qty.\n"
        "Do NOT leave part_number blank for sub-items — read it from the part number column.\n"
        "Extract ALL rows (numbered + un-numbered) in a SINGLE top-to-bottom pass.\n"
        "If rows_extracted < " + str(total_rows) + ", you missed numbered rows — check each page image again."
    )
