    ...
    if bom_extracted:
        if using_images and text_source == "image_only":
            ...
            page_instruction = (
                ...
                "=== SHX CHARACTER CONFUSION GUIDE ===\n"
                "SHX fonts draw characters as thin line segments. These pairs look\n"
                "nearly identical — examine EACH character with extreme care:\n"
                "  D ↔ O ↔ 0  (all oval shapes — look for the flat left edge on D,\n"
                "               perfectly round = O or 0, context: letters→O, digits→0)\n"
                "  L ↔ 1      (both vertical with a base — L has a longer horizontal\n"
                "               foot, 1 has a shorter serif or none)\n"
                "  U ↔ V      (U has a rounded bottom, V is pointed — look carefully\n"
                "               at the bottom curve. Do NOT drop small characters.)\n"
                "  S ↔ 5      (nearly identical in SHX! context: letters→S, digits→5)\n"
                "  B ↔ 8      (B has flat left edge, 8 is fully rounded)\n"
                "  6 ↔ 8      (6 has open top, 8 is closed)\n"
                "  I ↔ 1      (context: letters→I, digits→1)\n"
                "  C ↔ O      (C has an opening on the right, O is closed)\n"
                "  E ↔ F      (E has bottom horizontal, F does not)\n"
                "  3 ↔ 5      (3 has two right-facing curves, 5 has a top bar)\n"
                "CRITICAL: When you see a character that could be two things,\n"
                "look at the SURROUNDING characters for context. In a letter\n"
                "sequence use the letter variant; in a digit sequence use the digit.\n"
                "Example: 'SCE-727212ULP' — the U between 2 and L is a letter.\n"
                "Example: 'DA30D0F' — between D and F, the oval is 0 (digit context).\n\n"
                "=== CRITICAL: READ THE IMAGE — DO NOT INVENT ===\n"
                "RULE 1 — Read character-by-character. Copy EXACTLY what is shown.\n"
                "Do NOT substitute similar part numbers from your training data.\n"
                "RULE 2 — Never skip rows: Return a JSON entry for EVERY numbered row.\n"
                "RULE 3 — Never drop characters. If a part number has 13 characters\n"
                "in the image, your output must have 13 characters. Count them.\n"
